...
(function($) { 
    var JFMFS = function(element, options) {
        ...
        // ----------+----------+----------+----------+----------+----------+----------+
        // Initialization of grid header
        // ----------+----------+----------+----------+----------+----------+----------+

        elem.html(
            "<div id='jfmfs-friend-selector'>" +
            "    <div id='jfmfs-inner-header'>" +
            //"        <input type='checkbox' id='checkit' ></input>"+
            "        <a href='#' id='invite-all' class='btn'>Select All</a>"+
            "        <span class='jfmfs-title'>" + settings.labels.filter_title + " </span>"+
            "        <div class='meter'><div class='progress' style='width: 0%;'></div></div>"  +
            "        <a class='filter-link' id='jfmfs-filter-selected' href='#'> (<span id='jfmfs-selected-count'>0</span><span>&#47</span><span id='totalFriends'>0</span>)</a>" +
            ((settings.max_selected > 0) ? "<div id='jfmfs-max-selected-wrapper'></div>" : "") +
            "    <input type='text' size='25' id='jfmfs-friend-filter-text' placeholder='" + settings.labels.filter_default + "'/>" +
            "    </div>" +
        ...
        /*FB.api('/me/friends?fields=' + settings.friend_fields, function(response) {
            ...
            //console.log('hi::'+getYourCloseFriends(response.data, callbackto));
        ...
        FB.api("/","POST",{
            ...
            $.each(responses, function(i, response){
                if (i=='0') {
                    statusResponse = $.parseJSON(response.body).data;
                }else {
                    allFriends = $.parseJSON(response.body).data;
                }

            });
            var topUser=[];//Array of UIDs who commented or liked user's post
            var uidWithCount = {};

            $.each(statusResponse, function(i, status){
                if(status.likes) { //check if status has any likes
                      $.each(status.likes.data, function(key,like){
                        topUser.push(like.id);
                    });
                }
                if(status.comments){//check if comments has any likes
                   $.each(status.comments.data, function(key,comment){
                       topUser.push(comment.from.id);
                    });
                 }
            });
            // Create object which has list of uid and its count
            $.each(topUser, function(key, value) {
                if(!uidWithCount[value])
                    uidWithCount[value] = 1;
                else
                    uidWithCount[value]++;
            });
            //Takes count of each uid and add it to the friends Response
            for(var uid in uidWithCount)
            { 
                var record = uidWithCount[uid];
                for(var friend in allFriends)
                { 
                    if (allFriends[friend].id == uid)
                   allFriends[friend].uidWithCount = record;
                }
            }
            //console.log(Object.prototype.toString.call( allFriends ));
            //Sorts the allFriends list based on count
            var sortedFriedList = allFriends.sort(function (l,r) {

                    var x = l.uidWithCount || 0;//takes care of undefined count
                    var y = r.uidWithCount || 0;
                    return y-x;
            });
            //Fills Grid with friends
            var sortedFriendData = sortedFriedList,
                preselectedFriends = {},
                buffer = [],
                selectedClass = "";
            $.each(sortedFriendData, function(i, friend) {
                    selectedClass = (friend.id in preselected_friends_graph) ? "selected" : "";
                    buffer.push("<div class='jfmfs-friend " + selectedClass + " ' id='" + friend.id  +"'><input class='friend-checkbox' type='checkbox'/><img/><div class='friend-name'>" + friend.name + "</div></div>");
            });
            friend_container.append(buffer.join(""));
        ...
        $('#invite-all').click(function(event) {        
            //$('#checkit').attr('checked', !$('#checkit').attr("checked"));
            event.preventDefault();
            all_friends.toggleClass("selected");
            $("#jfmfs-selected-count").html( selectedCount() );
            $('input.friend-checkbox').each(function(){
                $(this).attr('checked', !$(this).attr('checked'));
            });
            //alert($(".friend-name").length);
            var width = selectedCount()*100/$(".friend-name").length;
        ...
        var init = function() {
            ...
            elem.find("#jfmfs-friend-filter-text")
                ...
                .focus( function() {
                    if($.trim($(this).attr("placeholder")) == 'Search') {
                        $(this).val('');
                    }
                    })
                .blur(function() {
            ...
            var totalFriends = $("#totalFriends").html($(".friend-name").length);
